...
export class NextScriptCustom extends NextScript {
  render() {
    ...
    let initialLoadScripts = scripts.filter(
      ({ props }) => props && initialFilterer(props)
    );
    const nextDataScripts = initialLoadScripts.filter(
      ({ props }) => props && nextDataFilter(props)
    );
    initialLoadScripts = initialLoadScripts.filter(
      ({ props }) => props && !nextDataFilter(props)
    );
    ...
    const polyfillJSX = polyfill.map(({ props }) => (
      <script
        key={`initialScript${props.src}${props.id}`}
        {...props}
        src={props.src}
      />
    ));
    const innerScriptsJSX = generateInnerScript(
      chunkedScripts,
      calculatedDelay
    );
    ...
    return (
      <>
        ...
        <script
          ...
          defer
          dangerouslySetInnerHTML={{ __html: innerScriptsJSX }}
        />
